...
- **RESOLVED 2026-06-21 (O-AIR-01).** DAG now publishes Gold to `gold/_current/`. `gold_run_id(ts_nodash)` derives one shared `run_id` per DAG run (no XCom); new `dbt_seed` task; `dq_checks()` split into `dbt_test` (against unpublished `gold/<run_id>/`) → `publish_gold` (`scripts/publish_gold.py --run-id`) → `dq_validate` (GE against `gold/_current/` only). Implements ADR-005 Decision 1 mechanism (B) as already specified — no new pattern, no ADR amendment. Independently re-verified by senior data engineer review: real MWAA parse gate clean, full MinIO `gym-lake` rep (seed→staging→snapshot→marts→serving→test 54P/1W/0E→publish→GE PASS), idempotent re-publish confirmed, `dim_drug`=133,654 (no P-PMR-07 regression). Two non-blocking risks noted (partial-publish-on-mid-loop-failure mitigated by per-model independence + Airflow retries; no `gold/<run_id>/` retention policy) — accepted, not gating. See SIGN_OFF_LOG.md 2026-06-21.
